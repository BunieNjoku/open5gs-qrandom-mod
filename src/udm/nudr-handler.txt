...
// Helper function to write the server response to a string
    ...
    udm_ue_t *udm_ue, ogs_sbi_stream_t *stream, ogs_sbi_message_t *recvmsg)
{
    ...
    CASE(OGS_SBI_RESOURCE_NAME_AUTHENTICATION_SUBSCRIPTION)
        ...
        CASE(OGS_SBI_HTTP_METHOD_PATCH)
            ...
            // Replacing ogs_random with a call to the server
            char rand_string[OGS_KEYSTRLEN(OGS_RAND_LEN)]; // buffer for storing hex string
            char rand_hex[OGS_RAND_LEN * 2 + 1] = {0}; // buffer to store server response
            CURL *curl;
            CURLcode res;

            curl = curl_easy_init();
            if(curl) {
                curl_easy_setopt(curl, CURLOPT_URL, "http://localhost:5000/random");
                curl_easy_setopt(curl, CURLOPT_WRITEFUNCTION, write_callback);
                curl_easy_setopt(curl, CURLOPT_WRITEDATA, rand_hex);
                ...
                ogs_error("Failed to initialize CURL");
                return false;
            }

            // Convert hex string to binary
            ogs_ascii_to_hex(rand_hex, strlen(rand_hex), udm_ue->rand, sizeof(udm_ue->rand));
